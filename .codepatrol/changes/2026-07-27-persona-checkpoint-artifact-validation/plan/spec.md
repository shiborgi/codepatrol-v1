...
- Mode: bug
- Target baseline: `main` at `08a43e5e85f5c617ba4d4b0d7abc89e6f7f03d85` (the Change's recorded immutable `base_commit`); Change branch `codepatrol/2026-07-27-persona-checkpoint-artifact-validation`; clean tree at start; `npm run verify` green.
...
- Expected surface delta: modify `src/change/validation.ts`, `src/change/orchestrator.ts`, `src/change/orchestrator-parallel.test.ts`, and `src/change/change.test.ts`; no new file, dependency, configuration, or public interface beyond one new optional, defaulted parameter on two existing functions.
