...
- AC-6: The Kanban (`scripts/render-kanban.mjs --format markdown`) renders a first "Backlog" column; backlog-only items appear as rows with lifecycle cells `-`; a promoted item's row shows its priority in the Backlog cell and its lifecycle progress in the stage cells.
