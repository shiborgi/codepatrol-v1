...
- Directly confirmed `resolveInside(workspace, ".codepatrol/runtime/traces/../../escape-marker.jsonl")` returns `<workspace>/.codepatrol/escape-marker.jsonl` without throwing: `resolveInside` proves only workspace containment, so routing `trace.path` through `resolveInside` alone (with no further check) would still accept the inside-workspace escape case above. The fix must check containment against the resolved *traces root*, not only the workspace root.
...
1. `src/shared/state.ts`: add `tracePath(workspace: string, workId: string): string`. `resolveInside` alone only proves containment within the *workspace* root; a `workId` containing `..` segments can still resolve to a workspace-contained path that lands outside `.codepatrol/runtime/traces/` (confirmed by direct execution: `resolveInside(workspace, ".codepatrol/runtime/traces/../../escape-marker.jsonl")` returns `<workspace>/.codepatrol/escape-marker.jsonl`, which `resolveInside` accepts because it never leaves the workspace). `tracePath` therefore resolves both the traces root and the candidate through `resolveInside`, then additionally rejects a candidate whose path relative to the *traces root* is absolute or starts with `..`, throwing `CodepatrolError("INVALID_WORKSPACE", ...)`. This two-layer check (workspace containment via `resolveInside`, then traces-subtree containment via an explicit `relative()` check against the resolved traces root) is required because `resolveInside` is a workspace-boundary primitive, not a nested-directory-boundary primitive. Same private `RUNTIME_DIR` constant already used by `graphStatePath`, `lockPath`, and `stageSessionPath`; no new exported constant.
...
- Every trace file a running command can produce resolves to a path inside `<workspace>/.codepatrol/runtime/traces/` specifically, not merely somewhere inside the workspace — verified through `resolveInside` (workspace containment) plus an explicit `relative()` check against the resolved traces root (traces-subtree containment). No trace-related code path can create or open a file outside `.codepatrol/runtime/traces/`.
...
- Every currently valid work id — the collision-safe `YYYY-MM-DD-slug` shape `change start` generates, and any other id that does not contain `..` or otherwise resolve outside `.codepatrol/runtime/traces/` — produces byte-identical trace paths and behavior to before this change.
...
- Observability: a containment violation on `append`/`appendRaw` now surfaces on stderr as `[trace] append failed: Path escapes the traces directory: ...` (inside-workspace escape) or `[trace] append failed: Path escapes the workspace: ...` (full workspace escape, from `resolveInside` itself) instead of silently succeeding outside `.codepatrol/runtime/traces/` — a strictly more informative failure mode, not a new one, since these functions already print a failure message for any other I/O error today.
...
- Risk: relying on `resolveInside` alone (workspace-root containment) without an additional traces-root-relative check would still accept an inside-workspace escape from `.codepatrol/runtime/traces/` — directly confirmed by executing `resolveInside` against the AC-1 input and observing it return `<workspace>/.codepatrol/escape-marker.jsonl` without throwing. Mitigation: `tracePath` performs the additional `relative(tracesRoot, candidate)` check described in Proposed design point 1; both the inside-workspace and full-workspace escape cases are separately red-tested in T2.
