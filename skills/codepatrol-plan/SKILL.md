...
When investigation shows the work exceeds one bounded Change, call `codepatrol backlog add --input -` for each follow-up (`source.kind: "plan-followup"`, `source.workId: <this-work-id>`) so each is queryable, deduplicated, and surfaced by `next --stage plan` and the Kanban Backlog column. After calling `backlog add`, the caller MUST commit `.codepatrol/backlog/items.yaml` (`git add .codepatrol/backlog/ && git commit -m <message>`) before the next Change transition; the `backlog add` CLI does not commit on its own — it only writes the file — so an uncommitted backlog before a checkpoint produces `CHANGE_CONFLICT: Checkpoint has undeclared worktree paths: .codepatrol/backlog/items.yaml`.
